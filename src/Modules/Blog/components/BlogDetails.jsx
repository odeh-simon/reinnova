...
const BlogDetails = () => {
  ...
  const [postContent, setPostContent] = useState(
    sessionStorage.getItem(`postContent-${id}`) || ""
  );
      ...
      const fetchPost = async () => {
        try {
          const response = await axios.get(
            `https://www.googleapis.com/blogger/v3/blogs/${blogId}/posts/${id}?key=${API_KEY}`
          );
          const contentWithoutImage = extractImageAndContent(
            ...
            <div className="flex flex-col-reverse md:flex-row gap-4 md:gap-[80px] md:items-start">
              ...
              <div className="prose prose-lg max-w-none w-full lg:w-[75%]">
                ...
                <p className="text-sm font-[poppins] mt-6 py-4 border-black border-t-2">
                  <Link
                    to={"/about-us"}
                    className="text-[#016E97] font-semibold"
                  >
                    Learn more{" "}
                  </Link>
                  about Reinnova Green&apos;s sustainable solutions for
                  packaging, plastic waste management, and aluminium products.
                </p>
              </div>
            </div>
          </>
